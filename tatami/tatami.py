import logging
from typing import NoReturn, Optional, Self, Union
...
class Tatami(Router):
    ...
    def run(self, host: str = 'localhost', port: int = 8000, openapi_url: Optional[str] = '/openapi.json', swagger_url: Optional[str] = '/docs/swagger', redoc_url: Optional[str] = '/docs/redoc', rapidoc_url: Optional[str] = '/docs/rapidoc') -> NoReturn:
        ...
        if openapi_url is not None:
            self.routes.insert(0, Route(openapi_url, openapi_endpoint, methods=["GET"]))

            if redoc_url is not None:
                self.routes.insert(0, Route(redoc_url, redocs_endpoint, methods=["GET"]))
            
            if swagger_url is not None:
                self.routes.insert(0, Route(swagger_url, swagger_endpoint, methods=["GET"]))
            
            if rapidoc_url is not None:
                self.routes.insert(0, Route(rapidoc_url, rapidoc_endpoint, methods=["GET"]))
